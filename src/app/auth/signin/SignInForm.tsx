...
export function SignInForm({ providers }: Props) {
  ...
  const oauthProviders = Object.values(providers ?? {}).filter((p) => p.id !== 'email');

  if (!emailProvider && oauthProviders.length === 0) {
    return (
      <p className="rounded-md border border-yellow-500/20 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-400">
